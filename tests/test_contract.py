#!/usr/bin/env python3
"""
Contract Tests for template-architect-aget
...
class TestArchitectArchetype:
    def test_archetype_is_architect(self):
        archetype = get_template_root() / ".aget" / "persona" / "archetype.yaml"
        if archetype.exists():
            assert "architect" in archetype.read_text().lower()

    def test_governance_is_rigorous(self):
        """Architect must have rigorous governance."""
        archetype = get_template_root() / ".aget" / "persona" / "archetype.yaml"
        if archetype.exists():
